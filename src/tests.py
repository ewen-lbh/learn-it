...
class IntegrationTests(unittest.TestCase):
    def setUp(self):
        self.base_dir = path(os.path.dirname(os.path.dirname(__file__)), 'test-data', 'schoolsyst')
        self.extractor = extract.Extractor(path(self.base_dir, 'english', 'verbes-irreguliers.adoc'))
        self.extractor.TITLE_FMT = '%(Subject)s: %(notion)s'
        with open(path(self.base_dir,'expected-extracted-learndata.yaml')) as f:
            self.expected_dict = dict(yaml.load(f.read(), Loader=yaml.BaseLoader))
    
    def test_destination(self):
        actual = self.extractor._dst
        expected = path(self.base_dir, 'english', 'learndata', 'verbes-irreguliers.yaml')
        self.assertEqual(actual, expected)
    ...
    def test_files(self):
        ...
        with open(self.extractor._dst, 'r') as f:
            actual = f.read()
        with open(path(self.base_dir, 'english', 'learndata', 'verbes-irreguliers.yaml'), 'r') as f:
            expected = f.read()
        self.assertEqual(actual, expected)
