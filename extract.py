...
class Extractor():
    ...
    def __init__(self, src, dst=None, file_format=None):
        ...
        if dst is None:
            ...
            self._dst = os.path.join(
                ...
                # folder/learndata/[file-name].yaml
        ...
        if file_format is None:
            ...
            # ext[:1] because the '.' is in the extension part
            markdown = ext[1:] in ('md', 'markdown', 'mdown')
    ...
    def get_infos(self):
        ...
        try:
            ...
            notion = os.path.splitext(os.path.split(self._src)[1])[
                0].replace('-', ' ')

        self.infos = {
            # get subject from blah/foo/spam/subject/file-name.adoc
            'subject': os.path.split(os.path.dirname(self._src))[1],
            'notion': notion
        }
    ...
    def compute_flags(self):
        ...
        for k, v in self.infos.items():
            ...
            if len(v) > 1:
                ...
                ask_sentence_fmt_params[k[0].upper(
                )+k[1:]] = v[0].upper() + v[1:]
            else:
    ...
    def compute_data(self):
        ...
        for line in self._raw.split('\n'):
            if not re.match(r'^#no-learndata(:.+(/.+)?)?', line):
    ...
    def save_file(self):
        ...
        serialized = yaml.dump(complete_data, Dumper=yaml.CDumper, default_flow_style=False, allow_unicode=True)
    ...
    def extract(self):
        ...
        if self.list_count:
            print(
                f"Detected {colored(self.list_count, color='cyan')} learndata-marked list(s).")
        else:
            cprint('No learndata-marked lists.', color='red')
            print(
                f"Check out how to mark lists as learndata here: {colored('https://schoolsyst.com/help/mark-as-learndata', color='cyan')}")
